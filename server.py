...
def register():
    ...
    try:
        ...
        return jsonify({"status": "fail", "message": "이미 존재하는 사용자"})
...
def login():
    ...
    with get_db() as db:
        ...
        if not user:
            return jsonify({"status": "fail", "message": "사용자 없음"})
        if not bcrypt.checkpw(password.encode(), user["password"].encode()):
            return jsonify({"status": "fail", "message": "비밀번호 불일치"})

        if not user["expiry"]:
            return jsonify({"status": "fail", "message": "관리자 승인 대기중"})

        if datetime.today() > datetime.strptime(user["expiry"], "%Y-%m-%d"):
            return jsonify({"status": "fail", "message": "사용기간 만료"})
...
def set_expiry():
    ...
    try:
        ...
        if payload["username"] != "admin":
            return jsonify({"status": "fail", "message": "권한 없음"})

        data = request.json
        with get_db() as db:
            db.execute("UPDATE users SET expiry = ? WHERE username = ?", (data["expiry"], data["username"]))
        return jsonify({"status": "success", "message": "기간 설정 완료"})
    except Exception:
        return jsonify({"status": "fail", "message": "인증 실패"})
...
def me():
    ...
    try:
        ...
        return jsonify({"status": "fail", "message": "토큰 인증 실패"})
